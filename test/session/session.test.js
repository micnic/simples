...
tap.test('Session.prototype.constructor()', (test) => {
	...
	test.equal(typeof session.expires, 'number');
...
tap.test('Session.prototype.load()', (test) => {
	...
	test.test('Non-existent id', (t) => {
		TestUtils.mockCryptoRandomBytes(null, Buffer.from([255]), (restore) => {
			const session = new Session('non-existent', {
				store: new Store(),
				timeout: 1
			});
	...
	test.test('Existent id', (t) => {
		...
		const session = new Session('existent-id', {
			store,
			timeout: 1
		});
		...
		session.load().then(() => {
			...
			t.equal(typeof session.expires, 'number');
...
tap.test('Session.prototype.save()', (test) => {
	...
	const store = new Store({
		set(id, s) {
			...
			test.equal(id, 'id');
			test.equal(s, JSON.stringify(session));
