...
var Server = function (port, options) {
	...
	this.instance.on('request', this.requestListener);
	this.instance.on('upgrade', this.upgradeListener);
...
Server.prototype.mirror = function (port, options, callback) {
	...
	if (this.port !== port && !this.mirrors[port]) {
