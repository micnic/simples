...
server.prototype.handle = function (request, response) {
	...
	if (contentEncoding) {
		...
		responseStream = zlib['create' + contentEncoding[0].toUpperCase() + contentEncoding.slice(1)]();;
		responseStream.response = response;
	...
	var requestInterface = {
		...
		get cookies() {
			...
			if (request.headers.cookie) {
				request.headers.cookie.split(/\s*;\s*/).forEach(function (element, index, array) {
		...
		if (request.method === 'GET') {
			if (this.routes.get[requestURL]) {
				...
				fs.stat(realPath, function (error, stats) {
					if (!error && stats.isFile()) {
						...
						that.routes.error[404](requestInterface, responseInterface);
					}
				});
			} else {
				response.statusCode = 404;
				this.routes.error[404](requestInterface, responseInterface);
			}
			...
			request.on('end', function () {
				...
				if (contentType === 'application/x-www-form-urlencoded' || contentType === 'text/plain') {
					...
					that.routes.error[400](requestInterface, responseInterface);
					return;
				}
				if (that.routes.post[requestURL]) {
					that.routes.post[requestURL](requestInterface, responseInterface);
				} else if (that.routes.all[requestURL]) {
					that.routes.all[requestURL](requestInterface, responseInterface);
				} else {
					response.statusCode = 404;
					that.routes.error[404](requestInterface, responseInterface);
				}
			...
			this.routes.error[405](requestInterface, responseInterface);
		}
	} catch (error) {
		response.statusCode = 500;
		this.routes.error[500](requestInterface, responseInterface);
