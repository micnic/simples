...
server.prototype.handle = function (request, response) {
	...
	requestInterface.cookies = (function () {
		var result = {};
		if (request.headers.cookie) {
			request.headers.cookie.split(/\s*;\s*/).forEach(function (element) {
				...
				if (name === '_session') {
					requestInterface.session = that.sessions[value];
				} else {
					result[name] = value;
				}
			});
	...
	requestInterface.langs = (function () {
	...
	if (!requestInterface.session) {
		...
		requestInterface.session = this.sessions[name];
		response.setHeader('Set-Cookie', '_session=' + name + ';expires=' + new Date(new Date().valueOf() + 3600000).toUTCString() + ';path=/;domain=' + request.headers.host + ';httponly');
	} else {
		response.setHeader('Set-Cookie', '_session=' + requestInterface.session._name + ';expires=' + new Date(new Date().valueOf() + 3600000).toUTCString() + ';path=/;domain=' + request.headers.host + ';httponly');
		clearTimeout(requestInterface.session._timeout);
		requestInterface.session._timeout = setTimeout(function () {
			delete that.sessions[requestInterface.session._name];
		}, 3600000);
	}
