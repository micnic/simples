...
var requestInterface = function (request) {
	'use strict';

	var host = request.server.hosts[request.headers.host] || request.server.hosts.main;
	...
	if (request.method === 'POST') {
		...
		request.on('end', function () {
			...
			if (contentType === 'multipart/form-data') {
				...
				var items = this.body.split(new RegExp('\r?\n?--' + boundary + '-?-?\r\n'));
				items.shift();
				items.pop();
				items.forEach(function (element) {
					...
					if (fileNameIndex >= 0) {
						...
						filename = element.substr(0, element.indexOf('"'));
						element = element.slice(element.indexOf('Content-Type: ') + 14);
						type = element.substr(0, element.indexOf('\r'));
					}
	...
	if (request.headers.cookie) {
		request.headers.cookie.split(/\s*;\s*/).forEach(function (element) {
			...
			if (name === '_session') {
				this.session = host.sessions[value];
			} else {
...
var responseInterface = function (responseStream) {
	'use strict';
...
responseInterface.prototype.cookie = function (name, value, config) {
	...
	if (config) {
		...
		if (config.domain) {
			...
			cookie += ';domain=' + this.request.headers.host;
		}
...
server.prototype.handle = function (request, response) {
	...
	if (request.url === '/simples/client.js' && request.method === 'GET') {
		response.end(client);
		return;
	}
	...
	try {
		...
		try {
			...
			responseStream.end('"' + request.url + '" Internal Server Error');
		}
...
server.prototype.routing = function (routeInterface) {
	'use strict';

	var request = routeInterface[1].stream.request;
	var response = routeInterface[1].stream.response;
	var responseStream = routeInterface[1].stream;
