...
class MultipartParser extends Transform {
	...
	constructor(contentType) {
		...
		// Check for boundary to initialize parser members
		if (boundary) {
			...
			// Emit the error that there is no boundary found
			throw Error('No boundary found for multipart parser');
		}
	}
	...
	_flush(callback) {
		...
		if (this.expect === parsingFinished) {
			...
			callback(Error('Unfinished multipart data'));
		}
	}
	...
	endParsing(byte, callback) {
		if (byte === carriageReturn && this.prev === 0) {
			...
			this.expect = parsingFailed;
			callback(Error('Invalid multipart data ending'));
		}
	}
	...
	getData(data, byte, index, callback) {
		...
		if (byte === this.boundary[this.boundaryIndex]) {
			...
			if (byte === carriageReturn) {
				this.prev = carriageReturn;
				this.boundaryIndex++;
			} else if (byte === hyphenMinus) {
				this.prev = hyphenMinus;
				this.boundaryIndex++;
			} else {
				this.expect = parsingFailed;
				callback(Error('Unexpected boundary symbol found'));
			}
		} else if (this.boundaryIndex > this.boundaryLength) {
	...
	newPart(disposition, callback) {
		...
		if (title) {
			...
			this.expect = parsingFailed;
			callback(Error('Invalid content disposition structure'));
		}
	}
	...
	parseByte(chunk, byte, index, callback) {
		...
		if (this.expect === expectFirstBoundary) {
			...
			this.expect = parsingFailed;
			callback(Error('Invalid parser state'));
		}
	}
	...
	parseHeader(callback) {
		...
		const key = this.header.substr(0, index).trim().toLowerCase();
		const value = this.header.substr(index + 1).trim();
		...
		if (index > 0) {
			if (!field && key === 'content-disposition') {
				...
				this.expect = parsingFailed;
				callback(Error('Invalid header received'));
			}
		} else {
			this.expect = parsingFailed;
			callback(Error('No header name delimiter found'));
		}
	}
	...
	// Multipart data parser factory method
	static create(contentType) {

		return new MultipartParser(contentType);
	}
