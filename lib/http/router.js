...
const httpTimeout = 5000; // Milliseconds, default HTTP timeout
...
class Router extends EventEmitter {
	...
	constructor(parent, location) {
		...
		this._parent = parent;
		this._pattern = Router.getPattern(location);
	}
	...
	logger(format, config) {
		...
		if (typeof format === 'string') {
			...
			config = Object.assign({}, config, enable);
	...
	router(location, options) {
		...
		if (typeof location === 'string') {
			...
			const router = Router.create(this, location, options);

			// Add the router to the host's routers container based on its type
			if (Router.isDynamic(location)) {
	...
	timeout(value) {
		...
		// Set the numeric value of the timeout
		options.timeout = Config.getNumberOption(value, options.timeout);
	...
	ws(location, options, listener) {
		...
		if (typeof location === 'string') {
			...
			if (typeof listener === 'function') {
				return WSHost.create(this, location, options, listener);
			}
		}
	 ...
	 * @returns {Router}
	 */
	static create(parent, location, options) {

		const router = new Router(parent, location);

		// Set router options
		Router.setOptions(router, parent._options, options);
	...
	static optionsContainer(parentConfig, config) {
		...
		const timeout = Config.getNumberOption(parentConfig.timeout, config.timeout);
		...
		return {
			...
			timeout: Config.getNumberOption(timeout, httpTimeout)
		};
	}
}
