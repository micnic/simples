...
var form = function (request, config) {
	...
	if (type) {
		if (config.urlencoded && form.urlencodedRegExp.test(type)) {
			this.parser = new utils.parsers.qs();
			this.type = 'urlencoded';
		} else if (config.multipart && form.multipartRegExp.test(type)) {
			this.parser = new utils.parsers.multipart(type);
			this.type = 'multipart';
		} else if (config.json && form.jsonRegExp.test(type)) {
...
// RegExp to match json content type
form.jsonRegExp = /json/i;

// RegExp to match multipart content type
form.multipartRegExp = /multipart/i;

// RegExp to match urlencoded content type
form.urlencodedRegExp = /urlencoded/i;
