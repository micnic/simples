...
var host = function (parent, name, config) {
	...
	// Set interval to remove expired sessions
	setInterval(utils.http.clearSessions, 3600000, this).unref();
};
...
host.prototype.close = function () {
	...
	// Clear the WS hosts
	Object.keys(this.wsHosts).forEach(function (wsHost) {
		that.wsHosts[wsHost].close();
	});
...
host.prototype.config = function (config) {
	...
	// Transform session timeout to miliseconds
	if (config.session && typeof config.session.timeout === 'number') {
...
host.prototype.del = function (routes, callback) {
	this.route('del', routes, callback);
	return this;
};
...
host.prototype.route = function (verb, routes, callback) {
	...
	function addDynamicRoute(verb, route, callback) {
		...
		// Prepare named parameter matches
