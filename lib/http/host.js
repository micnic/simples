...
var host = function (parent, name) {
	...
	Object.defineProperties(conf, {
		compression: { // do compression or not, default true
			value: true,
			writable: true
		},
		limit: { // limit request body in bytes, default 1MB
			value: 1048576,
			writable: true
		},
		...
		session: { // the time to live for a session in seconds, default 1 hour
			value: 3600,
			writable: true
		}
	});
...
host.prototype.log = function (logger) {
	this.logger = logger;
	return this;
};
...
host.prototype.ws = function (location, config, callback) {
	...
	// Create the WebSocket host or change an existing one
	if (this.wsHosts[location]) {
		this.wsHosts[location].open(config, callback);
	} else {
