...
var connection = function (host, request, response) {
	...
	request.on('readable', function () {
		...
		that.body = Buffer.concat([that.body, this.read() || new Buffer(0)]);

		// Check for too big request body
		if (that.body.length > host.conf.limit) {
			console.error('\nsimpleS: Request Entity Too Large\n');
			that.body = new Buffer(0);
			this.destroy();
		}
	}).on('end', function () {

		// Convert buffer data to UTF-8 encoded string
		that.body = that.body.toString();

		// Route the requests
		utils.routing(host, that);
	});
