...
// RegExp to match deflate accepted content encoding
connection.deflateRegExp = /deflate/i;

// RegExp to match a dot in the domain name
connection.domainDotRegExp = /\./i;

// RegExp to match gzip accepted content encoding
connection.gzipRegExp = /gzip/i;

// RegExp to match text group in content type
connection.textGroupRegExp = /^text\//i;

// Set compression for HTTP connections
connection.setCompression = function (host, instance) {
	...
	if (encoding && compression.enabled && compression.filter.test(type)) {
		...
		deflate = connection.deflateRegExp.test(encoding);
		gzip = connection.gzipRegExp.test(encoding);
...
connection.setCORS = function (instance) {
...
connection.prepare = function (instance) {
	...
	// Add compression to the connection if needed
	connection.setCompression(instance.parent, instance);

	// Check for CORS requests
	if (instance.headers.origin) {
		connection.setCORS(instance);
	}
...
connection.prototype._flush = function (callback) {
	...
	// Check for the first write and prepare the connection
	if (!this.started) {
...
connection.prototype.cookie = function (name, value, attributes) {
	...
	if (attributes.domain) {
		...
		if (connection.domainDotRegExp.test(attributes.domain)) {
...
connection.prototype.type = function (type, override) {
	...
	if (typeof type === 'string') {
		...
		if (!override && connection.textGroupRegExp.test(type)) {
			type += ';charset=utf-8';
		}
