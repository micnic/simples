...
var connection = module.exports = function (host, request, response) {
	...
	function getSession() {
		...
		if (!session) {
			name = utils.generateSessionName();
		...
		if (!sessionWritten) {
			...
			this.cookie('_session', session._name, {
				domain: this.host,
				expires: 3600,
				httpOnly: true,
				path: '/'
...
connection.prototype.cookie = function () {
	'use strict';
	...
	// Prepare cookies parameters
	if (typeof arguments[1] === 'string') {
		cookie += encodeURIComponent(arguments[1]);
		attributes = arguments[2];
	} else {
		attributes = arguments[1];
		if (attributes) {
			attributes.expires = 0;
		}
	}

	// Use expires or max-age to set the expiration time of the cookie
	if (attributes.expires) {
		var timestamp = new Date().valueOf() + attributes.expires * 1000;
