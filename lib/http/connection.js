...
class HttpConnection extends AbstractConnection {
	...
	constructor(parentRouter, location, request, response) {
	...
	cookie(name, value, attributes) {
		...
		if (attributes.expires) {
			...
			const expires = TimeFormatter.utcFormat(attributes.expires);

			// Add expiration time to the cookie string
		...
		if (this._socket.encrypted && attributes.secure) {
			cookie += ';secure';
		}
	...
	log(data, logger, tokens) {
		...
		data = data.replace(/%req\[([^\]]+)]\b/g, (match, header) => {
			return this.request.headers[header];
		});

		// Replace response headers
		data = data.replace(/%res\[([^\]]+)]\b/g, (match, header) => {
			return this.response.getHeader(header);
		});

		super.log(data, logger, tokens);
	}
	...
	parse(config) {
		...
		if (config && typeof config === 'object') {
			HttpForm.parse(this.request, config);
		}
	...
	static applyCompression(connection, compression) {
		...
		const filter = compression.filter;

		// Check for enabled compression and filter connection
		if (compression.enabled && (!filter || filter(connection))) {

			const encoding = connection.headers['accept-encoding'];
			const deflate = /deflate/i.test(encoding);
			const gzip = /gzip/i.test(encoding);

			let responseStream = connection.response;

			// Check for supported compression
			if (deflate && (compression.preferred === 'deflate' || !gzip)) {
				connection.header('Content-Encoding', 'Deflate');
				responseStream = zlib.Deflate(compression.options);
			} else if (gzip && (compression.preferred === 'gzip' || !deflate)) {
				connection.header('Content-Encoding', 'Gzip');
				responseStream = zlib.Gzip(compression.options);
			}

			// Pipe the connection to the writable stream
	...
	// HTTP connection factory method
	static create(parentRouter, location, request, response) {

		return new HttpConnection(parentRouter, location, request, response);
	}

	// Prepare connection for writing process
	static start(connection) {

		const optionsContainer = connection._router._options;
		const compression = optionsContainer.compression;
		const cors = optionsContainer.cors;
