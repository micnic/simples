...
class HTTPConnection extends AbstractConnection {
	...
	log(format, logger, tokens) {
		...
		super.log(format, logger, tokens);
	}
	...
	static create(router, location, request, response) {

		return new HTTPConnection(router, location, request, response);
	}

	// Prepare connection for writing process
	static start(connection) {

		const { compression, cors } = connection._router._options;
