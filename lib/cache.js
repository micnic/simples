...
cache.prepareClient = function (instance) {
	...
	fs.stat(cache.client.location, function (error, stats) {
		if (error) {
			utils.emitError(instance, error, true);
		} else {
...
cache.prototype.addDirectory = function (object, location, callback) {
	...
	var rpath = '/' + path.relative(this.container.location, location),
		that = this;
	...
	fs.readdir(location, function (error, files) {
		if (error) {
			utils.emitError(that, error, true);
		} else {
...
cache.prototype.destroy = function () {
	if (this.container && this.container.files) {
		if (this.busy) {
...
cache.prototype.init = function () {
	...
	fs.stat(location, function (error, stats) {
		if (error) {
			...
			utils.emitError(that, new Error('Root is not a directory'), true);
		} else {
...
cache.prototype.read = function (location) {
	...
	while (object && index < slices.length && slices[index]) {
