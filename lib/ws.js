...
module.exports = function (request, socket, host) {
	...
	if (!(host && headers.origin && headers.upgrade === 'websocket' && headers['sec-websocket-version'] === '13' && headers['sec-websocket-protocol'])) {
		socket.destroy();
		return;
	}
	...
	var origin = url.parse(headers.origin).host;

	// Check for valid origins
	if (!(headers.host === origin || headers.origin === 'null' || ~this.origins.indexOf(origin) || (this.origins[0] === '*' && !~this.origins.indexOf(origin)))) {
		socket.destroy();
		return;
	}

	// Check for WebSocket protocols equality
	var protocols = headers['sec-websocket-protocol'].split(/\s*,\s*/).sort();
	if (protocols < config.protocols || protocols > config.protocols) {
		socket.destroy();
		return;
	}
