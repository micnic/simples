...
module.exports = function (request, socket, host) {
	...
	if (headers.origin) {
		...
		index = ~this.origins.indexOf(origin);
	}
	...
	var activeHost = host && host.started;
	/*var validHeaders = headers.upgrade !== 'websocket' || headers['sec-websocket-version'] !== '13' || !headers['sec-websocket-protocol']*/
	if (!activeHost ||
		!headers.origin ||
	...
	try {
		host.callback.call(host, connection);
	} catch (error) {
		console.log('\nsimpleS: error in WebSocket host\n' + error.message + '\n');
	}

	// Data state containers
	var closeData = new Buffer([136, 0]);
	var pingData = new Buffer([137, 0]);
