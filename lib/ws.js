...
var wsConnection = function (request, host) {
	'use strict';
...
var ws = module.exports = function (request, host) {
	'use strict';
	...
	if (!(headers.host === url.parse(headers.origin).hostname && (headers.origin === 'null' || ~this.origins.indexOf('*') || ~this.origins.indexOf(url.parse(headers.origin).hostname)))) {
		socket.destroy();
		return;
	}
	...
	var connection = new wsConnection(request, host);
