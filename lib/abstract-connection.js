...
class AbstractConnection extends Transform {
	...
	log(format, logger, tokens) {
		...
		// Make format argument optional
		if (typeof format === 'function') {

			// Make looger argument optional
		...
		// Set default format
		if (!format) {
			format = '%short-date %time %method %href';
		}
		...
		if (tokens && typeof tokens === 'object') {
			Object.keys(tokens).forEach((token) => {
				...
				if (typeof replacer === 'function') {
					format = format.replace(RegExp(`%${token}\\b`, 'g'), replacer);
				}
			});
		}

		// Replace defined tokens
		format = format.replace(/%date\b/g, String(date));
		...
		format = format.replace(/%timestamp\b/g, Number(date));
