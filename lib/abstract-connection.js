...
class AbstractConnection extends Transform {
	...
	log(format, tokens, logger) {
		...
		const day = `0${date.getDate()}`.slice(-dateFormatSlice);
		const hours = `0${date.getHours()}`.slice(-dateFormatSlice);
		const minutes = `0${date.getMinutes()}`.slice(-dateFormatSlice);
		const month = `0${date.getMonth() + 1}`.slice(-dateFormatSlice);
		const seconds = `0${date.getSeconds()}`.slice(-dateFormatSlice);
